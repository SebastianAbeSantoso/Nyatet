...
- The elicited evaluation set will probably stay small and authored, rather than taken from real operations.
- Benchmark-domain evaluation doesn't exercise the normalizer or the resolver, so the benchmark numbers don't represent end-to-end accuracy.
- Event spans are the weakest class on the benchmark (EVT 0.388 F1).
- Latency was measured on Kaggle CPU, not on the machine the demo will run on.
...
Once the serving layer is built, the whole MVP is one endpoint.
...
```
POST /parse
{ "message": "indomi 1 dus sm gula 5 kg" }

{
  "order_lines": [{
    "item_span": "indomi",
    "item_matched_name": null,
    "tagging_confidence": 0.97,
    "resolution_confidence": 0.13,
    "qty_raw": "1", "qty_normalized": 1.0,
    "unit_raw": "dus", "unit_normalized": "dus"
  }],
  "anaphoric_flags": [{ "span_text": "yg biasa" }]
}
```
...
| [docs/RESULTS.md](docs/RESULTS.md) | Measured results: benchmark methodology, metrics, model comparisons, and performance |
